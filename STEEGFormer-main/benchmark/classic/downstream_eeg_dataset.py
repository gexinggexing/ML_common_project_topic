...
class DTUDataset(torch.utils.data.Dataset):
    def __init__(self, data_path, fold=0, regression_task="dtu", segment_time = 3, new_fs=256, hop_t=0.1, train=True, class_label=None, transform=None, chan_info=None):
        """
        Args:
        ...
        self.train = train
        if self.train:
            self.hop_len = int(hop_t*self.new_fs)
        else:
            self.hop_len = int(4/self.orginal_fs*self.new_fs) #64 hz prediction 
    ...
    def __getitem__(self, idx):
        ...
        wav_win = self.wav_recordings[trail_idx][:,pos + self.data_length]
...
class BinocularSSVEPDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        freqbands: list = [(5,95),(12,95),(19,95),(27,95),(35,95)] 
    ):
        self.subjectName      = os.path.splitext(os.path.basename(data_path))[0]
        self.fold             = fold
        ...
        self.transform        = transform
        self.chan_info        = chan_info
        self.freqbands        = freqbands  # e.g., [(7,90), (14,90), ...] — assume used at preprocessing time

        # --- Load pre-filtered data ---
        with open(data_path, "rb") as f:
            sub = pickle.load(f)
        self.data = sub["data"]  # shape: (T, E, C, F, L)

        # --- Metadata ---
        self.df = sub["df"]
        train_idx, sync_idx, async_idx = sub["splits"][fold]
        ...
        if self.train:
            ...
            if self.task == "sync":
                sync_train_idx = []
                for i in train_idx:
                    if self.df.iloc[i]['start_sample'] == 0:
                        sync_train_idx.append(i)
                self.example_idx = sync_train_idx
        elif self.task == "sync":
            self.subjectName += "_sync"
            self.example_idx = sync_idx
        else:
            self.subjectName += "_async"
    ...
    def __getitem__(self, idx):
        ...
        # Expect pre-filtered shape: (C, F, L)
        arr = self.data[t, e, :, :, s : s + self.data_length]

        # Add batch dimension: (1, C, F, L)
        arr_batch = arr[np.newaxis, ...]

        if self.transform:
            arr_batch = self.transform(arr_batch)

        arr_out = np.squeeze(arr_batch, axis=0)  # shape: (C, F, L)
