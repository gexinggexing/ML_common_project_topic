...
#SBATCH --job-name=error_task%a # set a task name
...
conda activate env1 # environment for each downstream tasks

# 2) Define grid
downstream_tasks=(error) # choose a downstream task name
evaluation_schemes=(population leave-one-out-finetuning per-subject) # set the evaluation schemes. 3 options: population leave-one-out-finetuning per-subject
models=(xdawn_lda xdawncov_mdm xdawncov_ts_svm erpcov_mdm dcpm) # the models used on this task. Check model list in utils.py -> get_ML_models()
