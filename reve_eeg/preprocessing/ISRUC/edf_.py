...
class RawEDF(BaseRaw):
    ...
    def __init__(
        ...
        *,
        ...
        info, edf_info, orig_units = _get_info(input_fname, stim_channel, eog, misc, exclude, infer_types, preload, include)
        ...
        if units is None:
            ...
            units = dict.fromkeys(info["ch_names"], units)
...
class RawGDF(BaseRaw):
    ...
    def __init__(
        ...
        logger.info("Extracting EDF parameters from {}...".format(input_fname))
        input_fname = os.path.abspath(input_fname)
        info, edf_info, orig_units = _get_info(input_fname, stim_channel, eog, misc, exclude, True, preload, include)
        logger.info("Creating raw.info structure...")
        ...
        self.set_annotations(Annotations(onset=onset, duration=duration, description=desc, orig_time=None))
...
def _read_header(fname, exclude, infer_types, include=None):
    ...
    if ext in ("bdf", "edf", "rec"):
        ...
        raise NotImplementedError(f"Only GDF, EDF, and BDF files are supported, got {ext}.")


def _get_info(fname, stim_channel, eog, misc, exclude, infer_types, preload, include=None):
    ...
    sfreq = np.take(n_samps, not_stim_ch).max() * edf_info["record_length"][1] / edf_info["record_length"][0]
    del n_samps
    ...
    if lowpass.size == 0:
        ...
        warn("Channels contain different lowpass filters. Lowest filter setting will be stored.")
    ...
    edf_info["offsets"] = edf_info["physical_min"] - edf_info["digital_min"] * edf_info["cal"]
...
def _parse_prefilter_string(prefiltering):
    ...
    highpass = np.array([v for hp in [re.findall(r"HP:\s*([0-9]+[.]*[0-9]*)", filt) for filt in prefiltering] for v in hp])
    lowpass = np.array([v for hp in [re.findall(r"LP:\s*([0-9]+[.]*[0-9]*)", filt) for filt in prefiltering] for v in hp])
...
def _read_edf_header(fname, exclude, infer_types, include=None):
    ...
    with open(fname, "rb") as fid:
        ...
        try:
            meas_date = datetime(year, month, day, hour, minute, sec, tzinfo=timezone.utc)
        except ValueError:
            warn(f"Invalid date encountered ({year:04d}-{month:02d}-{day:02d} {hour:02d}:{minute:02d}:{sec:02d}).")
            meas_date = None
            ...
            for ch_label in ch_labels:
                ...
                if len(parts) > 1:
                    if parts[0].upper() in CH_TYPE_MAPPING:
                        ...
                        logger.info(f"Channel '{ch_label}' recognized as type {ch_type} (renamed to '{ch_name}').")
        ...
        physical_min = np.array([float(_edf_str_num(fid.read(8))) for ch in channels])[sel]
        physical_max = np.array([float(_edf_str_num(fid.read(8))) for ch in channels])[sel]
        digital_min = np.array([float(_edf_str_num(fid.read(8))) for ch in channels])[sel]
        digital_max = np.array([float(_edf_str_num(fid.read(8))) for ch in channels])[sel]
...
def _read_gdf_header(fname, exclude, include=None):
    ...
    with open(fname, "rb") as fid:
        ...
        if edf_info["number"] < 1.9:
            ...
            if record_length[0] == 0:
                record_length[0] = 1.0
                warn("Header information is incorrect for record length. Default record length set to 1.")
            ...
            bytes_tot = np.sum([GDFTYPE_BYTE[t] * n_samps[i] for i, t in enumerate(dtype)])
        ...
        # ---------------------------------------------------------------------
            ...
            if birthday == 0:
                ...
                birthday = datetime(1, 1, 1, tzinfo=timezone.utc) + timedelta(birthday * pow(2, -32) - 367)
            ...
            patient["headsize"] = np.ma.masked_array(patient["headsize"], np.equal(patient["headsize"], 0), None).filled()
            ...
            # EVENT TABLE
            # -----------------------------------------------------------------
            etp = edf_info["data_offset"] + edf_info["n_records"] * edf_info["bytes_tot"]
...
def _check_stim_channel(stim_channel, ch_names, tal_ch_names=["EDF Annotations", "BDF Annotations"]):
    ...
    tal_ch_names_found = [ch for ch in valid_stim_ch_names if ch in [t.lower() for t in tal_ch_names]]
    if tal_ch_names_found:
        _msg = (
...
def _find_exclude_idx(ch_names, exclude, include=None):
    ...
    if include:  # find other than include channels
        if exclude:
            raise ValueError(f"'exclude' must be empty if 'include' is assigned. Got {exclude}.")
...
def _read_annotations_edf(annotations, ch_names=None, encoding="utf8"):
    ...
    for k, ev in enumerate(triggers):
        ...
        for description in ev[3].split("\x14")[1:]:
            if description:
                if "@@" in description and ch_names is not None and description.split("@@")[1] in ch_names:
